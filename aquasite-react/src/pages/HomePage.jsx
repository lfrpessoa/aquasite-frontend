...
const usePosts = () => {
  ...
  const deletePost = async (id) => {
    const currentUser = localStorage.getItem('currentUser');
    try {
      const res = await fetch(`https://aquasite-main-1.onrender.com/api/posts/${id}`, {
        method: 'DELETE',
...
const HomePage = () => {
  ...
  useEffect(() => {
    ...
    const fetchUnread = async () => {
      try {
        const res = await fetch(`https://aquasite-main-1.onrender.com/api/notifications/${currentUser}/unread-count`);
        const data = await res.json();
